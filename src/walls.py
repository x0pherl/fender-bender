...
def wall_channel(length: float) -> Part:
    ...
    with BuildPart() as channel:
        ...
        with BuildSketch(Plane.XY.offset(config.wall_thickness * 3)):
            Rectangle(config.wall_thickness + config.tolerance * 2, length)
        loft()
        with BuildPart(
            Plane.XY.offset(config.wall_thickness), mode=Mode.SUBTRACT
        ):
            Box(
                config.wall_thickness + config.tolerance * 2,
                length,
        ...
        with BuildPart(Plane.XY.offset(config.wall_thickness * 2)):
            with GridLocations(
                config.wall_thickness + config.tolerance * 2,
                (length + config.wall_thickness / 2) / 2,
                2,
                2,
            ):
    ...
    with BuildPart() as tongue:
        ...
        with BuildPart(mode=Mode.SUBTRACT):
            ...
            with BuildPart(Location((0, 0, config.wall_thickness * 0.75))):
                Sphere(radius=config.wall_thickness * 0.75)
                Cylinder(
                    radius=config.wall_thickness * 0.6,
...
def guide_side(length: float) -> Part:
    ...
    with BuildPart() as side:
        Box(
            config.minimum_structural_thickness - config.tolerance,
            length,
            config.wall_thickness * 3,
            align=(Align.CENTER, Align.CENTER, Align.MIN),
        )
...
def sidewall_divots(length: float = config.sidewall_straight_depth):
    ...
    with BuildPart() as divots:
        with BuildPart(Location((0, 0, config.wall_thickness))):
            with GridLocations(
                config.sidewall_width - config.wall_thickness * 2,
                length / 2,
                2,
                2,
            ):
                Sphere(radius=config.frame_click_sphere_radius)
        with GridLocations(
            config.sidewall_width - config.wall_thickness * 2, length / 2, 2, 2
        ):
            Sphere(radius=config.frame_click_sphere_radius)
    return divots.part
...
def sidewall(
    length: float = config.sidewall_section_depth,
    reinforce=False,
    flipped=False,
) -> Part:
    """
    ...
    with BuildPart() as wall:
        ...
        if reinforce:
            with BuildPart():
                ...
                extrude(
                    amount=config.minimum_structural_thickness
                    + config.wall_thickness
                )
        if not config.solid_walls:
            ...
            with BuildPart(mode=Mode.SUBTRACT):
                ...
                with BuildPart(mode=Mode.INTERSECT):
                    ...
                    add(hw)
        with BuildPart(
            Location((0, -config.sidewall_straight_depth / 2, 0)),
            mode=Mode.SUBTRACT,
        ):
...
def guide_wall(length: float, flipped=False) -> Part:
    ...
    with BuildPart() as wall:
        ...
        with BuildPart(wall.faces().sort_by(Axis.Y)[0]):
            add(straight_wall_tongue())
        with GridLocations(
            config.frame_bracket_spacing, 0, config.filament_count + 1, 1
        ):
...
if __name__ == "__main__":
    ...
    reinforced_side_wall = sidewall(
        length=config.sidewall_section_depth, reinforce=True
    )
    export_stl(reinforced_side_wall, "../stl/wall-side-reinforced.stl")
    export_stl(gwall, "../stl/wall-guide.stl")

    show(
        gwall.move(Location((0, -config.sidewall_straight_depth / 2, 0))),
        side_wall.move(
            Location(
                (
