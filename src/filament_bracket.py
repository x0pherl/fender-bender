...
def wheel_guide_cut() -> Part:
    ...
    with BuildPart() as wheelcut:
        base_radius = config.wheel_radius + config.wheel_radial_tolerance
        with BuildSketch():
...
def spoke_assembly() -> Part:
    ...
    with BuildPart() as constructed_brace:
        add(cut_spokes())
        Cylinder(
            radius=config.bearing_shelf_radius,
            height=config.bearing_shelf_height,
            align=(Align.CENTER, Align.CENTER, Align.MIN),
        )
        Cylinder(
            radius=config.bearing_inner_radius,
            height=config.bracket_depth / 2 - config.wheel_lateral_tolerance,
            align=(Align.CENTER, Align.CENTER, Align.MIN),
        )
...
def bracket_clip_rail_block(
    inset=0, frame_depth=config.frame_clip_point.y
) -> Part:
    """
    ...
    with BuildPart() as rail_block:
        with BuildPart(
            ...
            with BuildPart(railbox.faces().sort_by(Axis.Z)[-1]):
                with GridLocations(0, config.frame_clip_width - inset, 1, 2):
                    Box(
                        config.bracket_depth * 2 - inset,
                        config.frame_clip_rail_width - inset,
                        config.frame_clip_rail_width - inset,
                        rotation=(45, 0, 0),
                        align=(Align.CENTER, Align.CENTER, Align.CENTER),
                    )
        ...
        with BuildPart(railbox.faces().sort_by(Axis.X)[0]) as rounded:
            Cylinder(
                radius=(config.minimum_structural_thickness - inset) / 2,
        ...
        with BuildPart(
            Location(
                (
                    config.frame_bracket_exterior_x_distance(top_height)
                    - (
                        config.frame_click_sphere_radius
            ...
            with GridLocations(
                0,
                config.frame_clip_width - inset - config.wall_thickness,
                1,
                2,
            ):
                Cylinder(
                    radius=config.frame_click_sphere_radius - inset,
        ...
        with BuildPart(
            ...
            if inset > 0:
                Cylinder(
                    radius=max(
                        config.frame_click_sphere_radius / 2 + inset, 0
                    ),
...
def bracket_clip(inset=0, frame_depth=config.frame_clip_point.y) -> Part:
    ...
    with BuildPart() as clip:
        ...
        with BuildPart(
            Location(
                (
                    config.frame_clip_point.x + config.bracket_depth / 2,
                    0,
            ...
            Cylinder(
                radius=(config.frame_clip_width - config.fillet_radius * 2)
                / 3.5,
...
def bottom_bracket_block() -> Part:
    ...
    with BuildPart() as arch:
        ...
        fillet(arch.edges(), config.fillet_radius)
        with BuildPart(
            Location((config.wheel_radius, 0, 0)), mode=Mode.SUBTRACT
        ):
...
def pin_channel() -> Part:
    ...
    with BuildPart() as channel:
        add(
            rounded_cylinder(
                radius=base_unit,
                height=config.bracket_depth + config.tolerance,
                align=(Align.CENTER, Align.CENTER, Align.MIN),
            )
        )
        with BuildPart(Location((0, 0, -config.bracket_depth / 2))) as guide:
            Cylinder(
                radius=config.bracket_depth,
                height=base_unit * 2,
                rotation=(0, 90, 0),
            )
...
def bottom_bracket(draft: bool = False) -> Part:
    ...
    with BuildPart() as constructed_bracket:
        ...
        with BuildPart(mode=Mode.SUBTRACT):
            add(
                top_cut_template(config.tolerance)
                .mirror()
                .move(Location((0, 0, config.bracket_depth)))
            )
            Cylinder(
                radius=config.wheel_radius + config.wheel_radial_tolerance,
                height=config.bracket_depth,
                align=(Align.CENTER, Align.CENTER, Align.MIN),
            )
            with BuildPart(
...
if __name__ == "__main__":
    ...
    show(
        bottom.move(Location((config.bracket_width / 2 + 5, 0, 0))),
