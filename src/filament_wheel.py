...
def spoke() -> Sketch:
    ...
    """
    spoke_outer_radius = (
        config.wheel_radius + config.bearing_radius + config.rim_thickness
    ) / 2
    spoke_shift = (
        config.wheel_radius - config.bearing_radius - config.rim_thickness
    ) / 2
...
def filament_wheel() -> Part:
    ...
    with BuildPart() as wheel:
        with BuildSketch():
            ...
            Circle(
                radius=config.wheel_radius - config.rim_thickness,
                mode=Mode.SUBTRACT,
            )
        with BuildSketch():
        ...
        add(
            diamond_torus(config.wheel_radius, config.bearing_depth / 2).move(
                Location((0, 0, config.bearing_depth / 2))
            ),
            mode=Mode.SUBTRACT,
        )
    return wheel.part
...
if __name__ == "__main__":
    ...
    export_stl(part, "../stl/filament-bracket-wheel.stl")
